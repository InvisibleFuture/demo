<template>
<div id="P">
  <section class="P" @click.stop v-if="list.length > 0">
    ...
    <div class="P_title">
      ...
      <div class="clearfix" style="font-size: 12px; color: #7f7f7f;margin-right: -72px;position: relative;">
        来自版区：<a style="color: #ffaa00;" href="/main">未知</a>
        <!--
    ...
    <div class="P_main" v-for="item in list" v-html="item"></div>
      ...
      <ul>
        <li v-for="item in file">
          <a :href="item.path" :download="item.path">
            ...
            <span class="hash" v-text="'hash: '+item.hash"></span>
            <span class="count" v-text="item.count"></span>
            <span class="count">单独售价</span>
      ...
      <ul>
        <li v-for="item in post">
          ...
          <div>
            ...
            <span class="level">Lv{{ item.id }}</span>
            <span class="master" v-if="item.id == 22">Master</span>
            <span class="floor"># {{ item.id }}</span>
            <p>{{ item.message }}</p>
            <time>{{ item.time }}</time>
        ...
        <li>
          <div><img src="static/u1.jpg" onclick="gotoUserPage('/user6294360860113072660')" alt="username"></div>
          <div>
...
export default {
  ...
  data() {
    return {
      id: 0,
      title: 'Document name',
      list: ['Document text content'],
      time: 1398250549490,
      file: [
          { id:658, name:"奇怪的东西.zip", path:"6da4fas6.zip", hash:"ca6f46sc41f65a1c65af1ca614a56", count:9, time:1398250549490 },
          { id:659, name:"奇怪的东西.zip", path:"6da4fas6.zip", hash:"ca6f46sc41f65a1c65af1ca614a56", count:2, time:1398250549490 }
      ],
      tags: [
          { id:56, name:"TAGS" }
      ],
      post: [
          {id:223, content:"回帖内容", time:1398250549490 },
          {id:213, content:"回帖内容", time:1398250549490 }
      ],
      repy: [
          {id:515, content:"这是回复内容", time:1398250549490 }
      ],
      textarea:'请输入回复..'
    }
  },
  mounted: function() {
    ...
    // 给弹出层聚焦以支持使用Esc关闭, 但是好像独立的 P 没有 layer
  ...
  methods: {
    fetchData: async function() {
      ...
      server = ''
      axios.get( server + '/P-' + this.id + '.htm?ajax=1').then(r => {
        this.P = r.data.message.p,
