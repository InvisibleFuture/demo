...
<div class="hello">
  ...
  <nav class="navs">
    ...
    <ul>
      <li v-for="tab in tabs" v-bind:key="tab.id" v-bind:class="['tab-button', { active: currentTab === tab.id }]" v-on:click="currentTab = tab.id" :title="tab.title">{{ tab.name }}</li>
    </ul>
  </nav>
  ...
  data() {
    return {
      ...
      tabs: [{
          'id': 'trunk',
          'name': '大纲',
          'title': '结构组成/计划任务与完成度记录(主)'
        },
        {
          'id': 'issues',
          'name': 'issues',
          'title': '问题与疏漏与解决方案(细节方案)(主)/困扰/反面意见与权衡'
        },
        {
          'id': 'data',
          'name': '文档',
          'title': '文献资料与数据收集/文档/附件'
        },
        {
          'id': 'contributor',
          'name': '贡献者',
